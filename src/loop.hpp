...
public:
  ...
  void request_remove(const std::shared_ptr<async_descriptor> &descriptor) {
...
private:
  ...
  void call_handler(const std::shared_ptr<async_descriptor> &, io_events,
                    lifecycle_events);
  ...
  sync_queue<std::shared_ptr<async_descriptor>>
      m_add_queue; // queue to schedule add-tasks
  sync_queue<std::shared_ptr<async_descriptor>>
      m_remove_queue; // queue to schedule remove-tasks
