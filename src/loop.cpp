...
namespace rocket {
    ...
    io_loop::io_loop(int max_ready_events)
            : m_epoll_fd(::epoll_create1(0)), m_registered_fds(), m_add_queue(), m_remove_queue(),
              m_max_ready_events(max_ready_events),
    ...
    void io_loop::remove(const std::shared_ptr<rocket::async_descriptor> &descriptor_ptr) {

        if (!this->is_registered(descriptor_ptr)) return;

        int rc;

        rc = ::epoll_ctl(m_epoll_fd.get_fd(), EPOLL_CTL_DEL, descriptor_ptr->get_fd(), nullptr);
        if (rc) {
            throw std::system_error(errno, std::system_category());
        }
    ...
    void io_loop::call_handler(const std::shared_ptr<async_descriptor> &descriptor_ptr, io_events io,
                               lifecycle_events lc) {
    ...
    void io_loop::start() {
        ...
        std::unique_ptr<struct epoll_event[]> ready_fds(new struct epoll_event[m_max_ready_events]);
        ...
        do {
            ...
            rc = ::epoll_wait(m_epoll_fd.get_fd(), ready_fds.get(), m_max_ready_events, timeout);
            ...
            if (rc == -1 && errno != EINTR) {
                ...
                for (int i = 0; i < rc; ++i) {
                    ...
                    this->call_handler(descriptor_ptr, events, static_cast<lifecycle_events>(0));
                }
            }
        ...
        for (auto iter = m_registered_fds.cbegin(); iter != m_registered_fds.cend();) {
    ...
    int io_loop::check_timeout() {
        ...
        for (const auto &pair : m_registered_fds) {
            ...
            if (timeout.count() >= 0) {
                ...
                auto expiration = std::chrono::duration_cast<std::chrono::milliseconds>(diff).count();
