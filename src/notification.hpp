...
public:
  ...
  explicit notify_descriptor(
      std::chrono::milliseconds timeout = INFINITE_TIMEOUT)
      : notify_descriptor(std::static_pointer_cast<event_handler>(
                              std::make_shared<default_notification_handler>()),
                          timeout) {}
...
public:
  ...
  void on_io(io_loop &loop, const std::shared_ptr<async_descriptor> &fd,
    ...
    if (read) {
      auto &notifier = static_cast<notify_descriptor &>(*fd);
      uint64_t message;
      if (notifier.read(&message)) {
        static_cast<subclass_type *>(this)->on_notification(loop, fd, message);
      }
    }
  }
};
...
struct default_notification_handler
  ...
  void on_notification(io_loop &loop,
